/* $Id: walls.c,v 1.16 2008/10/12 15:45:14 rotunda_pk Exp $
 *
...
static void Walldist_alloc(void)
{
	...
	walldist = (uint8_t **) malloc(World.x * sizeof(uint8_t *)
			+ World.x * World.y);
	if (!walldist) {
...
static void Walldist_init(void)
{
	...
	while (qfront != qback) {
		...
		for (dx = -1; dx <= 1; dx++) {
			if (BIT(World.rules->mode, WRAP_PLAY) || (x + dx >= 0
					&& x + dx < World.x)) {
				wx = WRAP_XBLOCK(x + dx);
				for (dy = -1; dy <= 1; dy++) {
					if (BIT(World.rules->mode, WRAP_PLAY)
...
static void Move_segment(move_state_t *ms)
{
	...
	enter = ms->pos;
	if (enter.cx < 0 || enter.cx >= World.cwidth || enter.cy < 0
	...
	if (walldist[block.x][block.y] > 2) {
		int32_t maxcl = ((walldist[block.x][block.y] - 2) * BLOCK_CLICKS)
				>> 1;
		if (maxcl >= sign.x * ms->todo.cx && maxcl >= sign.y
				* ms->todo.cy) {
			...
			ms->done.cy = ms->todo.cy * maxcl / (sign.x
					* ms->todo.cx);
		}
		else {
			/* vertical movement. */
			ms->done.cx = ms->todo.cx * maxcl / (sign.y
					* ms->todo.cy);
	...
	else if (enter.cx == World.cwidth - 1 && !mi->edge_wrap && ms->vel.x
			> 0) {
	...
	else if (enter.cy == World.cheight - 1 && !mi->edge_wrap && ms->vel.y
			> 0) {
	...
	case TREASURE:
		...
		if (block_type == TREASURE) {
			if (mi->treasure_crashes) {
				...
				if (offset.cy > r && off2.cy > r && sqr(mid.cx
						- r) + sqr(mid.cy - r) > sqr(r)
						&& sqr(off2.cx - r) + sqr(
								off2.cy - r)
								> sqr(r)
						&& sqr(offset.cx - r) + sqr(
				...
				if (ms->treasure == mi->obj->treasure) {
					...
					if (!pl	|| (pl->team != mi->obj->treasure->team)) {
					...
					if (mi->obj->life != 0) {
						mi->obj->life = 0; /* time out the ball */
						...
						sprintf(
								msg,
								" < %s (team %d) has replaced the treasure >",
								pl->name,
								pl->team->Num);
						Set_message(msg);
						Rank_saved_ball(pl);
				...
				if ((ms->treasure->team == mi->obj->owner->team)
						&& (mi->obj->life != 0)) {
					...
					sprintf(
							msg,
							" < The ball was loose for %ld frames >",
							LONG_MAX
									- mi->obj->life);
					Set_message(msg);
					if (enemies > 0) {
					...
					Rank_ballrun(pl, LONG_MAX
							- mi->obj->life);
					Punish_team(mi->obj->owner,
							mi->obj->treasure,
							ms->treasure);
				}
	...
	case FILLED:
		...
		if (!ms->todo.cx && (offset.cx == 0 || offset.cx
				== BLOCK_CLICKS)) {
			/* tricky */
			break;
		}
		if (!ms->todo.cy && (offset.cy == 0 || offset.cy
				== BLOCK_CLICKS)) {
			/* tricky */
			break;
		}
	...
	case REC_LD:
		...
		if (sign.x * ms->todo.cx >= sign.y * ms->todo.cy) {
			...
			delta.cx
					= (int32_t) ((BLOCK_CLICKS - offset.cx
							- offset.cy) / (1 + w));
...
void Move_object(object_t *obj)
{
	...
	if (dist > 2) {
		...
		if (sqr(max) >= sqr(obj->vel.x) + sqr(obj->vel.y)) {
			int32_t cx = obj->pos.cx + FLOAT_TO_CLICK(obj->vel.x);
			int32_t cy = obj->pos.cy + FLOAT_TO_CLICK(obj->vel.y);
			cx = WRAP_XCLICK(cx);
			cy = WRAP_YCLICK(cy);
	...
	for (;;) {
		...
		if (!(ms.done.cx | ms.done.cy)) {
			...
			if (ms.bounce && ms.bounce != BounceEdge) {
				 ...
				 */
				if (!BIT(obj->status, FROMBOUNCE) && BIT(
						obj->type, OBJ_SPARK))
					CLR_BIT(obj->status, OWNERIMMUNE);
				if (sqr(ms.vel.x) + sqr(ms.vel.y) > sqr(
				...
				ms.todo.cx = (int32_t) (ms.todo.cx
						* objectWallBrakeFactor);
				ms.todo.cy = (int32_t) (ms.todo.cy
						* objectWallBrakeFactor);
			}
			if (++nothing_done >= 5) {
...
void Move_object_interpolation(object_t *obj)
{
	...
	if (dist > 2) {
		 ...
		 * the next frame.
		 */
		if (sqr(max) >= sqr(obj->vel_interp.x * speedfactor) + sqr(
				obj->vel_interp.y) * speedfactor) {
			int32_t cx = obj->pos_interp.cx + FLOAT_TO_CLICK(obj->vel_interp.x*speedfactor);
			int32_t cy = obj->pos_interp.cy + FLOAT_TO_CLICK(obj->vel_interp.y*speedfactor);
			cx = WRAP_XCLICK(cx);
			cy = WRAP_YCLICK(cy);
...
static void Player_crash(move_state_t *ms, int32_t pt, bool turning)
{
	...
	const int8_t *howfmt = NULL;
	const int8_t *hudmsg = NULL;
	...
	if (howfmt && hudmsg) {
		...
		if (num_pushers == 0) {
			...
			SCORE(pl, -sc, OBJ_X_IN_BLOCKS(pl), OBJ_Y_IN_BLOCKS(pl), hudmsg);
			strcat(msg, ".");
			Set_message(msg);
		}
		else {
			int32_t msg_len = strlen(msg);
			int8_t *msg_ptr = &msg[msg_len];
			int32_t average_pusher_score = total_pusher_score
					/ total_pusher_count;

			for (i = 0; i < num_pushers; i++) {
				player_t *pusher = pushers[i];
				const int8_t *sep = (!i) ? " with help from "
						: (i < num_pushers - 1) ? ", "
								: " and ";
				int32_t sep_len = strlen(sep);
				int32_t name_len = strlen(pusher->name);

				if (msg_len + sep_len + name_len + 2
						< sizeof msg) {
					strcpy(msg_ptr, sep);
					msg_len += sep_len;
					msg_ptr += sep_len;
				...
				sc = cnt[i] * (int32_t) floor(Rate(pusher->score,
						pl->score))
						/ total_pusher_count;
				SCORE(pusher, sc, OBJ_X_IN_BLOCKS(pl), OBJ_Y_IN_BLOCKS(pl), pl->name);
			}
			sc = (int32_t) floor(Rate(average_pusher_score, pl->score));
			SCORE(pl, -sc, OBJ_X_IN_BLOCKS(pl), OBJ_Y_IN_BLOCKS(pl), "[Shove]");
			strcpy(msg_ptr, ".");
			Set_message(msg);
		}
	}
...
void Move_player(player_t *pl)
{
	...
	for (;; moves_made++) {
		...
		if (walldist[block.x][block.y] > 3) {
			int32_t maxcl = ((walldist[block.x][block.y] - 3)
					* BLOCK_CLICKS) >> 1;
			todo = ms[0].todo;
			sign.x = (todo.cx < 0) ? -1 : 1;
			sign.y = (todo.cy < 0) ? -1 : 1;
			if (maxcl >= sign.x * todo.cx && maxcl >= sign.y
					* todo.cy) {
				/* entire movement is possible. */
				done.cx = todo.cx;
				done.cy = todo.cy;
			...
			if (ms[i].bounce) {
				if (bounce == -1) {
					...
					bounce = i;
						...
						== (ms[i].bounce == BounceEdge)) {
					if ((int32_t) (rfrac() * (ship->num_points
							- bounce)) == i) {
						bounce = i;
					}
				}
		...
		if (crash != -1) {
			...
			if (ms[worst].bounce != BounceEdge) {
				...
				/*int32_t	v = (int32_t) speed >> 2;
				 int32_t	m = (int32_t) (pl->mass - pl->emptymass * 0.75f);
				 DFLOAT	b = 1 - 0.5f * playerWallBrakeFactor;
				 int32_t	cost = (int32_t) (b * m * v);*/
				int32_t delta_dir, abs_delta_dir, wall_dir;
				DFLOAT
						max_speed =
								BIT(pl->used,
										OBJ_SHIELD) ? maxShieldedWallBounceSpeed
				...
				ms[worst].todo.cx = (int32_t) (ms[worst].todo.cx
						* playerWallBrakeFactor);
				ms[worst].todo.cy = (int32_t) (ms[worst].todo.cy
						* playerWallBrakeFactor);
				...
				if (pl->dir >= wall_dir) {
					delta_dir = (pl->dir - wall_dir <= RES
							/ 2) ? -(pl->dir
							- wall_dir) : (wall_dir
							+ RES - pl->dir);
				}
				else {
					delta_dir = (wall_dir - pl->dir <= RES
							/ 2) ? (wall_dir
							- pl->dir) : -(pl->dir
							+ RES - wall_dir);
				}
			...
			for (i = 0; i < ship->num_points; i++) {
				r[i].x = (vel.x) ? (DFLOAT) ms[i].todo.cx
						/ vel.x : 0;
				r[i].y = (vel.y) ? (DFLOAT) ms[i].todo.cy
						/ vel.y : 0;
				r[i].x = ABS(r[i].x);
				r[i].y = ABS(r[i].y);
			}
...
void Move_player_interpolation(player_t *pl)
{
	...
	if (BIT(pl->status, PLAYING | PAUSE | GAME_OVER | KILLED) != PLAYING) {
		if (!BIT(pl->status, KILLED | PAUSE)) {
			...
			pos.cx = WRAP_XCLICK(pos.cx);
			pos.cy = WRAP_YCLICK(pos.cy);
			if (pos.cx != pl->pos_interp.cx || pos.cy
					!= pl->pos_interp.cy) {
				Player_position_set_clicks_interpolation(pl,
						pos.cx, pos.cy);
			}
		}
		/*pl->velocity_interp = VECTOR_LENGTH(pl->vel_interp);*/
		pl->velocity_interp = hypot((double) (pl->vel_interp.x
				* speedfactor), (double) (pl->vel_interp.y
				* speedfactor));
		return;
	}
	...
	if (dist > 3) {
		...
		if (max >= pl->velocity_interp) {
			...
			pos.cx = WRAP_XCLICK(pos.cx);
			pos.cy = WRAP_YCLICK(pos.cy);
			Player_position_set_clicks_interpolation(pl, pos.cx,
					pos.cy);

			pl->velocity_interp = hypot((double) (pl->vel_interp.x
					* speedfactor),
					(double) (pl->vel_interp.y
							* speedfactor));
			/*pl->velocity_interp = VECTOR_LENGTH(pl->vel_interp);*/
			return;
		}
...
{
	pl->dir = MOD2((int32_t) (pl->float_dir + 0.5f), RES);
}
...
void Old_turn_player(player_t *pl)
{
	...
	move_state_t ms[RES];
	int32_t dir;
	int32_t new_dir = MOD2((int32_t) (pl->float_dir + 0.5f), RES);
	int32_t sign;
	int32_t crash = -1;
	int32_t nothing_done = 0;
