...
#define RADIUS 9
	...
	if (!sh.num_points) {
		...
		for (i = 0; i < MAX_SHIP_PTS; i++) {
			sh.pts[i] = &pts[i][0];
			sh.pts[i][0].x = (DFLOAT) RADIUS * cos(((double) i
					/ MAX_SHIP_PTS) * 2 * M_PI);
			sh.pts[i][0].y = (DFLOAT) RADIUS * sin(((double) i
					/ MAX_SHIP_PTS) * 2 * M_PI);
		}
...
#define GRID_RESET()	(memset(grid.pt, 0, sizeof grid.pt), \
	...
	if (shape_version == 0x3100) {
		...
		if (sscanf(str, "(%d,%d,%d)", &ofNum, &ofLeft, &ofRight) != 3
				|| ofNum < MIN_SHIP_PTS || ofNum > MAX_SHIP_PTS
				|| ofLeft < 0 || ofLeft >= ofNum || ofRight < 0
				|| ofRight >= ofNum) {
			if (verboseShapeParsing) {
		...
		for (i = 0; i < ofNum; i++) {
			...
			if (sscanf(str, "(%d,%d)", &inx, &iny) != 2) {
				if (verboseShapeParsing) {
					xpprintf(
							"Bad ship shape: format error in point %d\n",
							i);
				}
				return -1;
			}
	...
	if (!mainGunSet) { /* No main gun set, put at foremost point */
		...
		for (i = 1; i < w->num_points; i++) {
			if (pt[i].x > pt[max].x || (pt[i].x == pt[max].x
	...
	if (!w->num_l_light) { /* No left light set, put at leftmost point */
		max = 0;
		for (i = 1; i < w->num_points; i++) {
			if (pt[i].y > pt[max].y || (pt[i].y == pt[max].y
					&& pt[i].x <= pt[max].x)) {
				max = i;
			}
		}
	...
	if (shapeLimits) {
		const int32_t isLow = -8, isHi = 8, isLeft = 8, isRight = -8,
				minLow = 1, minHi = 1, minLeft = 1, minRight =
						1, horMax = 15, verMax = 15,
		...
		if (low < minLow || hi < minHi || left < minLeft || right
				< minRight || count < minCount) {
			if (verboseShapeParsing) {
				xpprintf(
			...
			|| (w->num_r_gun
					&& !(w->r_gun[0]
							= (position_t*) malloc(
									w->num_r_gun
											* i)))
			|| (w->num_l_rgun && !(w->l_rgun[0]
					= (position_t*) malloc(w->num_l_rgun
							* i)))
			|| (w->num_r_rgun && !(w->r_rgun[0]
					= (position_t*) malloc(w->num_r_rgun
							* i)))
			|| (w->num_l_light && !(w->l_light[0]
					= (position_t*) malloc(w->num_l_light
							* i)))
			|| (w->num_r_light && !(w->r_light[0]
					= (position_t*) malloc(w->num_r_light
							* i)))
			|| (w->num_m_rack && !(w->m_rack[0]
					= (position_t*) malloc(w->num_m_rack
							* i)))) {
...
void Free_ship_shape(shipshape_t *w)
{
	if (w != NULL && !w->default_ship) {
		if (w->num_points > 0 && w->pts[0])
			free(w->pts[0]);
		if (w->num_l_gun > 0 && w->l_gun[0])
			free(w->l_gun[0]);
		if (w->num_r_gun > 0 && w->r_gun[0])
			free(w->r_gun[0]);
		if (w->num_l_rgun > 0 && w->l_rgun[0])
			free(w->l_rgun[0]);
		if (w->num_r_rgun > 0 && w->r_rgun[0])
			free(w->r_rgun[0]);
		if (w->num_l_light > 0 && w->l_light[0])
			free(w->l_light[0]);
		if (w->num_r_light > 0 && w->r_light[0])
			free(w->r_light[0]);
		if (w->num_m_rack > 0 && w->m_rack[0])
			free(w->m_rack[0]);
#ifdef	_NAMEDSHIPS
		...
		uint32_t shape_version)
{
	...
	if (shape_version >= 0x3200) {
		...
		for (i = 0; i < w->num_points && i < MAX_SHIP_PTS; i++) {
			sprintf(&buf[buflen], " %d,%d", (int32_t) w->pts[i][0].x,
					(int32_t) w->pts[i][0].y);
			buflen += strlen(&buf[buflen]);
		}
		sprintf(&buf[buflen], ")(EN: %d,%d)(MG: %d,%d)",
				(int32_t) w->engine[0].x, (int32_t) w->engine[0].y,
				(int32_t) w->m_gun[0].x, (int32_t) w->m_gun[0].y);
		buflen += strlen(&buf[buflen]);
		...
		if (w->num_l_gun > 0) {
			...
			for (i = 0; i < w->num_l_gun && i < MAX_GUN_PTS; i++) {
				sprintf(&tmp[tmplen], " %d,%d",
						(int32_t) w->l_gun[i][0].x,
						(int32_t) w->l_gun[i][0].y);
		...
		if (w->num_r_gun > 0) {
			...
			for (i = 0; i < w->num_r_gun && i < MAX_GUN_PTS; i++) {
				sprintf(&tmp[tmplen], " %d,%d",
						(int32_t) w->r_gun[i][0].x,
						(int32_t) w->r_gun[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
			tmplen++;
		...
		if (w->num_l_rgun > 0) {
			...
			for (i = 0; i < w->num_l_rgun && i < MAX_GUN_PTS; i++) {
				sprintf(&tmp[tmplen], " %d,%d",
						(int32_t) w->l_rgun[i][0].x,
						(int32_t) w->l_rgun[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
			tmplen++;
		...
		if (w->num_r_rgun > 0) {
			...
			for (i = 0; i < w->num_r_rgun && i < MAX_GUN_PTS; i++) {
				sprintf(&tmp[tmplen], " %d,%d",
						(int32_t) w->r_rgun[i][0].x,
						(int32_t) w->r_rgun[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
			tmplen++;
		...
		if (w->num_l_light > 0) {
			...
			for (i = 0; i < w->num_l_light && i < MAX_LIGHT_PTS; i++) {
				sprintf(&tmp[tmplen], " %d,%d",
						(int32_t) w->l_light[i][0].x,
						(int32_t) w->l_light[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
			tmplen++;
		...
		if (w->num_r_light > 0) {
			...
			for (i = 0; i < w->num_r_light && i < MAX_LIGHT_PTS; i++) {
				sprintf(&tmp[tmplen], " %d,%d",
						(int32_t) w->r_light[i][0].x,
						(int32_t) w->r_light[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
			tmplen++;
		...
		if (w->num_m_rack > 0) {
			...
			for (i = 0; i < w->num_m_rack && i < MAX_RACK_PTS; i++) {
				sprintf(&tmp[tmplen], " %d,%d",
						(int32_t) w->m_rack[i][0].x,
						(int32_t) w->m_rack[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
			tmplen++;
		...
		int32_t num_points = MIN(w->num_points, 16);
#if 0
		...
		for (i = 1, ll = rl = 0; i < num_points; i++) {
			if (w->pts[i][0].y > w->pts[ll][0].y || (w->pts[i][0].y
					== w->pts[ll][0].y && w->pts[i][0].x
					< w->pts[ll][0].x)) {
				ll = i;
			}
			if (w->pts[i][0].y < w->pts[rl][0].y || (w->pts[i][0].y
					== w->pts[rl][0].y && w->pts[i][0].x
					< w->pts[rl][0].x)) {
				rl = i;
			}
		}
...
void Calculate_shield_radius(shipshape_t *w)
{
	...
	for (i = 0; i < w->num_points; i++) {
		radius2 = (int32_t) (sqr(w->pts[i][0].x) + sqr(w->pts[i][0].y));
		if (radius2 > max_radius) {
			max_radius = radius2;
		}
	}
	max_radius = (int32_t) (2.0 * sqrt(max_radius));
	w->shield_radius = (max_radius + 2 <= 34) ? 34 : (max_radius + 2
			- (max_radius & 1));
}
